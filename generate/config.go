...
type Config struct {
	...
	//
	// [genqlient.yaml docs]: https://github.com/obitech/genqlient/blob/main/docs/genqlient.yaml
	...
	if c.Package != "" {
		...
		if err != nil {
			return errorf(nil, "unable to guess package-name: %v"+
				...
				"\nExample: https://github.com/obitech/genqlient/blob/main/example/genqlient.yaml#L6", err)
		}
		...
		if !token.IsIdentifier(base) {
			return errorf(nil, "unable to guess package-name: '%v' is not a valid identifier"+
				"\nSet package name in genqlient.yaml"+
				"\nExample: https://github.com/obitech/genqlient/blob/main/example/genqlient.yaml#L6", base)
		}
